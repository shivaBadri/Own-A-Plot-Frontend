...
type Advantage = { title: string; body: string; image: string };

type AccentKey = "olive" | "earth" | "bark";
...
> = {
  olive: {
    dot: "bg-olive",
    activeText: "text-olive",
    ruleActive: "bg-olive",
  },
  earth: {
    dot: "bg-earth",
    activeText: "text-earth",
    ruleActive: "bg-earth",
  },
  bark: {
    dot: "bg-bark",
    activeText: "text-bark",
    ruleActive: "bg-bark",
  },
};
    ...
    <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
      <div className="lg:col-span-5">
        ...
        <h3 className="mt-6 font-serif text-h3">
          Why this ground, why now.
        </h3>
        <p className="prose-max mt-6 text-body text-muted">
          Hover a line on the right to see the ground it describes.
        </p>

        {/* Sticky image stage — desktop only. On mobile each advantage gets its own inline image. */}
        ...
        {advantages.map((a, i) => {
          ...
          return (
            <li
              ...
              tabIndex={0}
            >
              {/* Mobile-only inline image — desktop uses the sticky stage on the left. */}
